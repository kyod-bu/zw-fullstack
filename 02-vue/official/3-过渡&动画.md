...
CSS 动画用法同 CSS 过渡，**区别**是在动画中 `v-enter` 类名在节点插入 DOM 后不会立即删除，而是在 `animationend` 事件触发时删除。
...
对于原生标签可以使用 `v-if` / `v-else`。最常见的多 `<tag>` 过渡是一个列表和描述这个列表为空消息的元素：
...
多个组件的过渡简单很多，不需要使用 `key` attribute。相反，我们只需要使用**动态组件**：
...
这个看起来很神奇，内部的实现，Vue 使用了一个叫 [FLIP](https://aerotwist.com/blog/flip-your-animations/) 简单的动画队列，使用 transforms 将元素从之前的位置平滑过渡到新的位置。
...
我们能在组件中结合使用各种过渡策略和 Vue 内建的过渡系统。
